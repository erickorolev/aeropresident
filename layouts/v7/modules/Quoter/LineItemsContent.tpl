...
{foreach from = $SETTING  item = COLUMN}
    ...
    {elseif $COLUMN_NAME eq "quantity"}
        <td {if $COLUMN->isActive != 'active'}style="display: none;" {/if} class="cellItem tr_{$COLUMN_NAME}"  >
            ...
            {else}
                <input id="{$quantity}" name="{$quantity}" type="text" class="qty inputElement td_{$COLUMN_NAME}" style="width: 89%;"  value="{if !empty($data.$quantity)}{$data.$quantity}{else}1{/if}"/>
            {/if}
        </td>
                        ...
                        {if count($data.taxes) > 0}
                            {foreach key=tax_row_no item=tax_data from=$data.taxes}
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input style="width: 70px;" type="text"  name="{$taxname}" id="{$taxname}" value="{$tax_data.percentage}" class="smallInputBox taxPercentage inputElement" />&nbsp;%
                                    </td>
                                    <td>
            ...
            <div class="{$COLUMN_NAME}"  data-rowid="{$COLUMN_NAME}{$row_no}" data-lineitemtype = "{if !empty($data)}{$entityType}{else}{$BASE_ROW}{/if}">
                {if $EDITABLE eq "0"}
                    ...
                    {if is_array($data[$COLUMN_NAME|cat:$row_no]) || is_object($data[$COLUMN_NAME|cat:$row_no])}
                        ...
                        {if is_numeric($data[$FILED_NAME_ROW]->fieldvalue)}
                        ...
                        {/if}
                    {else}
                        {if is_numeric($data[$COLUMN_NAME|cat:$row_no])}
