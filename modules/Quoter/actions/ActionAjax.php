    ...
    public function getPSTemplates(Vtiger_Request $request)
    {
        ...
        if ($record) {
            ...
            foreach ($relatedProducts as $index => $product) {
                ...
                $info["defaultValue"][] = array($productId => array("id" => $productId, "name" => html_entity_decode($product["item_name" . $index], ENT_QUOTES, "utf-8"), "taxes" => $product["taxes"], "listprice" => $logSettings ? $product["listprice" . $index] : $product["unitPrice" . $index], "quantity" => $product["quantity" . $index], "discount_amount" => $product["discount_amount" . $index], "discount_percent" => $product["discount_percent" . $index], "listpricevalues" => array(), "description" => html_entity_decode($product["comment" . $index], ENT_QUOTES, "utf-8"), "quantityInStock" => 1, "type" => $product["entityType" . $index]));
                $customValues = array();
