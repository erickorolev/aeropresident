        ...
        if (!Vtiger_Field_Model::getInstance($fieldName, $moduleModel)) {
            ...
            if (!empty($custom_column_value["productField"])) {
                ...
                if ($product_field_model) {
                    $uitype = $product_field_model->uitype;
                    $typeofdata = $product_field_model->typeofdata;
                    $columntype = $product_field_model->columntype;
                }
            } else {
                if ($custom_column_value["serviceField"]) {
                    $service_field_model = Vtiger_Field_Model::getInstance($custom_column_value["productField"], Vtiger_Module_Model::getInstance("Services"));
                    if ($service_field_model) {
                        $uitype = $service_field_model->uitype;
                        $typeofdata = $service_field_model->typeofdata;
                        $columntype = $service_field_model->columntype;
                    }
                } else {
                    $uitype = 1;
                    $typeofdata = "V~O";
