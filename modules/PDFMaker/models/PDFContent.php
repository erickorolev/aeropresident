...
class PDFMaker_PDFContent_Model extends PDFMaker_PDFContentUtils_Model
{
    ...
    private static $inventory_table_array = array(
        "PurchaseOrder" => "vtiger_purchaseorder", "SalesOrder" => "vtiger_salesorder", "Quotes" => "vtiger_quotes",
        "Invoice" => "vtiger_invoice", "Issuecards" => "vtiger_issuecards", "Receiptcards" => "vtiger_receiptcards",
        "Creditnote" => "vtiger_creditnote", "StornoInvoice" => "vtiger_stornoinvoice"
    );
    private static $inventory_id_array = array(
        "PurchaseOrder" => "purchaseorderid", "SalesOrder" => "salesorderid", "Quotes" => "quoteid",
        "Invoice" => "invoiceid", "Issuecards" => "issuecardid", "Receiptcards" => "receiptcardid",
        "Creditnote" => "creditnote_id", "StornoInvoice" => "stornoinvoice_id"
    );
    private static $org_colsOLD = array(
        "organizationname" => "NAME", "address" => "ADDRESS", "city" => "CITY", "state" => "STATE", "code" => "ZIP",
        "country" => "COUNTRY", "phone" => "PHONE", "fax" => "FAX", "website" => "WEBSITE", "logo" => "LOGO"
    );
    ...
    public function getContent()
    {
        ...
        if (self::$type == "professional" || self::$type == "basic") {
            ...
            if ($this->focus->column_fields["assigned_user_id"] == "") {
                $this->focus->column_fields["assigned_user_id"] = self::$db->query_result(self::$db->pquery("SELECT smownerid FROM vtiger_crmentity WHERE crmid = ?",
                    array(self::$focus->id)), 0, "smownerid");
            }
            ...
            $this->convertHideTR();
            $PDF_content = array();
            list($PDF_content["header"], $PDF_content["body"], $PDF_content["footer"]) = explode(self::$section_sep,
                self::$content);
        } else {
            ...
            while ($row = self::$db->fetch_array($result)) {
                ...
                if ($related_module != "") {
                    ...
                    if ($fk_record != "" && $fk_record != "0") {
                        ...
                        $result_delete = self::$db->pquery("SELECT deleted FROM ".$control_sql."? AND deleted=0",
                            array($fk_record));
                ...
                if (trim($td->plaintext) == "#PRODUCTBLOC_".$block_type."START#") {
                    ...
                    if (strpos($var, "TOTAL") !== false) {
                        ...
                        if (isset($td->innertext)) {
                            list($style_subtotal_tag, $style_subtotal_endtag) = explode("#PRODUCTBLOC_".$block_type."START#",
                                $td->innertext);
                        }
    ...
    private function convertInventoryModules()
    {
        ...
        if ($num_rows > 0) {
            ...
            foreach ($Blocks as $block_type) {
                if (strpos(self::$content, "#PRODUCTBLOC_".$block_type."START#") !== false && strpos(self::$content,
                        "#PRODUCTBLOC_".$block_type."END#") !== false) {
    ...
    {
        return $this->getFieldValueUtils($efocus, $emodule, $fieldname, $value, $UITypes, $inventory_currency,
            self::$ignored_picklist_values, self::$def_charset, self::$decimals, self::$decimal_point,
            self::$thousands_separator, self::$language, self::$focus->id);
    }

    private function replaceFieldsToContent(
        $emodule,
        $efocus,
        $is_related = false,
        $inventory_currency = false,
        $related = "R_"
    ) {
        $current_user = Users_Record_Model::getCurrentUserModel();
                ...
                if (!empty($efocus->id)) {
                    ...
                    if ($fieldDataType == 'multipicklist') {
                        ...
                        if ($fieldDataType == 'reference' || $fieldDataType == 'owner') {
                            ...
                            if ($fieldDataType == 'double' || $fieldDataType == 'percentage') {
    ...
    private function replaceUserCompanyFields()
    {
        ...
        $tandc = self::$db->query_result(self::$db->pquery("SELECT tandc FROM vtiger_inventory_tandc WHERE type = ?",
            array('Inventory')), 0, "tandc");
        if (strpos($tandc, '&lt;br /&gt;') === false && strpos($tandc, '&lt;br/&gt;') === false && strpos($tandc,
                '&lt;br&gt;') === false) {
            self::$rep["$"."TERMS_AND_CONDITIONS$"] = nl2br($tandc);
        }
        if (self::$focus->column_fields["assigned_user_id"] != "") {
            $user_res = self::$db->pquery("SELECT * FROM vtiger_users WHERE id = ?",
                array(self::$focus->column_fields["assigned_user_id"]));
        ...
        $modifiedby_user_res = self::$db->pquery("SELECT vtiger_users.* FROM vtiger_users INNER JOIN vtiger_crmentity ON vtiger_crmentity.modifiedby = vtiger_users.id  WHERE  vtiger_crmentity.crmid = ?",
            array(self::$focus->id));
        ...
        $smcreatorid_user_res = self::$db->pquery("SELECT vtiger_users.* FROM vtiger_users INNER JOIN vtiger_crmentity ON vtiger_crmentity.smcreatorid = vtiger_users.id  WHERE  vtiger_crmentity.crmid = ?",
            array(self::$focus->id));
        ...
        if (count(self::$relBlockModules) > 0) {
            ...
            foreach (self::$relBlockModules as $relBlockModule) {
                if ($relBlockModule != "") {
                    ...
                    $rl_res = self::$db->pquery("SELECT vtiger_field.fieldlabel FROM vtiger_field INNER JOIN vtiger_tab ON vtiger_tab.tabid = vtiger_field.tabid WHERE vtiger_tab.name = ?",
                        array($relBlockModule));
    ...
    private function getTemplateData()
    {
        ...
        $result = self::$db->pquery("SELECT vtiger_pdfmaker.*, vtiger_pdfmaker_settings.* FROM vtiger_pdfmaker LEFT JOIN vtiger_pdfmaker_settings ON vtiger_pdfmaker_settings.templateid = vtiger_pdfmaker.templateid WHERE vtiger_pdfmaker.templateid=?",
            array(self::$templateid));
        $data = self::$db->fetch_array($result);
        self::$decimal_point = html_entity_decode($data["decimal_point"], ENT_QUOTES);
        self::$thousands_separator = html_entity_decode(($data["thousands_separator"] != "sp" ? $data["thousands_separator"] : " "),
            ENT_QUOTES);
    ...
    private function getInventoryProducts($module, $focus)
    {
        if (!empty($focus->id)) {
            ...
            $currencyFieldsList = array(
                'NETTOTAL' => 'hdnSubTotal', 'TAXTOTAL' => 'tax_totalamount', 'SHTAXTOTAL' => 'shtax_totalamount',
                'TOTALAFTERDISCOUNT' => 'preTaxTotal', 'FINALDISCOUNT' => 'discountTotal_final',
                'SHTAXAMOUNT' => 'shipping_handling_charge', 'DEDUCTEDTAXESTOTAL' => 'deductTaxesTotalAmount',
            );
            ...
            foreach ($relatedProducts as $i => $PData) {
                ...
                if ($comment != "") {
                    if (strpos($comment, '&lt;br /&gt;') === false && strpos($comment,
                            '&lt;br/&gt;') === false && strpos($comment, '&lt;br&gt;') === false) {
                        $comment = str_replace("
", "<br>", nl2br($comment));
                    }
                ...
                if (strpos($psdescription, '&lt;br /&gt;') === false && strpos($psdescription,
                        '&lt;br/&gt;') === false && strpos($psdescription, '&lt;br&gt;') === false) {
...
", "<br>", nl2br($psdescription));
                ...
                if (strpos($inventory_prodrel_desc, '&lt;br /&gt;') === false && strpos($inventory_prodrel_desc,
                        '&lt;br/&gt;') === false && strpos($inventory_prodrel_desc, '&lt;br&gt;') === false) {
                    $inventory_prodrel_desc = str_replace("
", "<br>", nl2br($inventory_prodrel_desc));
                }
                $Details["P"][$i]["CRMNOWPRODUCTDESCRIPTION"] = html_entity_decode($inventory_prodrel_desc, ENT_QUOTES,
                    self::$def_charset);
    ...
    {
        if (!empty($focus->id)) {
            $res = self::$db->pquery("SELECT taxtype FROM ".self::$inventory_table_array[$module]." WHERE ".self::$inventory_id_array[$module]."=?",
                array($focus->id));
    ...
    private function retrieve_entity_infoCustom(&$focus, $record, $module)
    {
        ...
        foreach ($focus->tab_name_index as $table_name => $index) {
            $result[$table_name] = self::$db->pquery("SELECT * FROM ".$table_name." WHERE ".$index."=?",
                array($record));
        }
        $tabid = getTabid($module);
        $result1 = self::$db->pquery("SELECT fieldname, fieldid, fieldlabel, columnname, tablename, uitype, typeofdata, presence FROM vtiger_field WHERE tabid=?",
            array($tabid));
...
"] = "";
            ...
            if (count($RelatedBlocks[1]) > 0) {
                ...
                foreach ($RelatedBlocks[1] as $relblockid) {
                    if (!in_array($relblockid, $ConvertRelBlock)) {
                        $secmodule = self::$db->query_result(self::$db->pquery("SELECT secmodule FROM vtiger_pdfmaker_relblocks WHERE relblockid = ?",
                            array($relblockid)), 0, "secmodule");
                        if (strpos(self::$content, "#RELBLOCK".$relblockid."_START#") !== false) {
                            if (strpos(self::$content, "#RELBLOCK".$relblockid."_END#") !== false) {
                                ...
                                $oRelBlockRun = new RelBlockRun(self::$focus->id, $relblockid, self::$module,
                                    $secmodule);
                                    ...
                                    foreach ($RelBlock_Data as $RelBlock_Details) {
                                        foreach ($ProductParts as $productpartid => $productparttext) {
                                            ...
                                            foreach ($RelBlock_Details as $coll => $value) {
                                                ...
                                                $productparttext = str_ireplace("$".$coll."$", $value,
                                                    $productparttext);
                                            }
                                            if ($show_line) {
                                                $ExplodedPdf[$productpartid] .= $productparttext;
    ...
    private function replaceInventoryDetailsBlock($module, $focus, $is_related = false)
    {
        ...
        $this->replaceContent();
        if ($is_related === false) {
            $TAX_TYPES = array("VATBLOCK", "DEDUCTEDTAXESBLOCK");
            foreach ($TAX_TYPES as $tax_type) {
                $vattable = "";
                if (count($Products["TOTAL"][$tax_type]) > 0) {
                    $vattable = "<table class='".strtolower($tax_type)."_style' border='1' style='border-collapse:collapse;' cellpadding='3'>";
                    $vattable .= "<tr>
                                    <td nowrap align='center'>".vtranslate("Name")."</td>
                                    <td nowrap align='center'>".vtranslate("LBL_VATBLOCK_VAT_PERCENT", "PDFMaker")."</td>
                                    <td nowrap align='center'>".vtranslate("LBL_VATBLOCK_SUM",
                            "PDFMaker")." (".$currencytype["currency_symbol"].")"."</td>
                                    <td nowrap align='center'>".vtranslate("LBL_VATBLOCK_VAT_VALUE",
                            "PDFMaker")." (".$currencytype["currency_symbol"].")"."</td>
                                  </tr>";
                    foreach ($Products["TOTAL"][$tax_type] as $keyW => $valueW) {
                        if ($valueW["netto"] != 0) {
                            $vattable .= "<tr>
            ...
            $this->replaceContent();
            $VProductParts = array();
            if (strpos(self::$content, "#VATBLOCK_START#") !== false && strpos(self::$content,
                    "#VATBLOCK_END#") !== false) {
                        ...
                        foreach ($VProductParts as $productpartid => $productparttext) {
                            if ($valueW["netto"] != 0) {
                                foreach ($valueW as $vColl => $vVal) {
                                    ...
                                    $productparttext = str_replace("$"."VATBLOCK_".strtoupper($vColl)."$", $vVal,
                                        $productparttext);
                                }
    ...
    {
        $Fields = array(
            "FIRSTNAME" => "first_name", "LASTNAME" => "last_name", "EMAIL" => "email1", "TITLE" => "title",
            "FAX" => "phone_fax", "DEPARTMENT" => "department", "OTHER_EMAIL" => "email2", "PHONE" => "phone_work",
            "YAHOOID" => "yahoo_id", "MOBILE" => "phone_mobile", "HOME_PHONE" => "phone_home",
            "OTHER_PHONE" => "phone_other", "SIGHNATURE" => "signature", "NOTES" => "description",
            "ADDRESS" => "address_street", "COUNTRY" => "address_country", "CITY" => "address_city",
            "ZIP" => "address_postalcode", "STATE" => "address_state"
        );
    ...
    public function getSettings()
    {
        ...
        $Settings["watermark"] = array(
            "type" => $Settings["watermark_type"], "text" => $Settings["watermark_text"],
